...
class Model {
    ...
    static async where(selector, options) {
        ...
        const models = await Promise.all(results.map(async row => {
            ...
            if(options.shouldInit && model.init) {
                await model.init()
            }
     ...
     */
    static async findBy(column, value, ...args) {
        return (await Model.where.call(this, `${column} = '${value}'`, ...args))[0]
    }

    /**
     * Create a collection from all matches for 'column = value'
     */
    static async findAllBy(column, value, ...args) {
        return await Model.where.call(this, `${column} = '${value}'`, ...args)
    }
